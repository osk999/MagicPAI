...
/// <summary>
/// Replay-determinism guard for <see cref="OrchestrateComplexPathWorkflow"/>.
/// See temporal.md §15.5. The orchestrator fans-out to
/// <see cref="ComplexTaskWorkerWorkflow"/> children so both types are registered.
/// </summary>
[Trait("Category", "Replay")]
public class OrchestrateComplexPathReplayTests
{
    [Theory]
    [InlineData("Workflows/Histories/orchestrate-complex-path/happy-path-v1.json")]
    public async Task ReplaysSuccessfully(string historyPath)
    {
        ...
        var history = Temporalio.Common.WorkflowHistory.FromJson(
            workflowId: "replay-orchestrate-complex-path",
            json: json);
