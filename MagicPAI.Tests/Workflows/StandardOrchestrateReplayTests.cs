...
/// <summary>
/// Replay-determinism guard for <see cref="StandardOrchestrateWorkflow"/>. See
/// temporal.md §15.5. The orchestrator dispatches a
/// <see cref="VerifyAndRepairWorkflow"/> child so both types are registered.
/// </summary>
[Trait("Category", "Replay")]
public class StandardOrchestrateReplayTests
{
    [Theory]
    [InlineData("Workflows/Histories/standard-orchestrate/happy-path-v1.json")]
    public async Task ReplaysSuccessfully(string historyPath)
    {
        ...
        var history = Temporalio.Common.WorkflowHistory.FromJson(
            workflowId: "replay-standard-orchestrate",
            json: json);
